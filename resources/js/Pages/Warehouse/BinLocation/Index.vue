    ...
    <AppLayout title="Bin Locations">
        ...
        <Modal :show="modal.open" @close="closeModal">
            <BinLocationForm
                ...
                @close="closeModal"
                @newBinLocation="onBinLocationCreated"
                @updateBinLocation="onBinLocationUpdated"
            />
        </Modal>
...
export default {
    ...
    setup() {
        ...
        const onBinLocationCreated = async (binLocation) => {
            loading.value = true;
            try {
                // If we have a selected warehouse, use that ID
                if (selectedWarehouse.value && !binLocation.warehouse_id) {
                    binLocation.warehouse_id = selectedWarehouse.value.id;
                }
                
                const result = await binLocationStore.createBinLocation(binLocation);
                ...
                binLocationStore.error = "Failed to create bin location";
            } finally {
                loading.value = false;
            }
        };

        const onBinLocationUpdated = async (binLocation) => {
            loading.value = true;
            try {
                const result = await binLocationStore.updateBinLocation(binLocation);
                ...
                binLocationStore.error = "Failed to update bin location";
            } finally {
                loading.value = false;
            }
        };
        ...
        return {
            ...
            closeDeleteDialog,
            confirmDelete,
            onBinLocationCreated,
            onBinLocationUpdated,
            goBackToWarehouses,
            handlePageChange,
            handleRowsChange,
            handleSearch
        ...
        return {
            ...
            startActions: [
                ...
                },
                {
                    ...
                    severity: "secondary",
                    size: "small",
                    command: () => this.goBackToWarehouses(),
                }
            ],
    ...
    methods: {
        ...
        onBinLocationCreated(binLocation) {
            this.$options.setup().onBinLocationCreated(binLocation);
        },
        onBinLocationUpdated(binLocation) {
            this.$options.setup().onBinLocationUpdated(binLocation);
        },
        goBackToWarehouses() {
