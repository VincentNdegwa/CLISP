...
<template>
    ...
    <ConfirmationModal :isOpen="confirmation.isOpen" :title="confirmation.title" :message="confirmation.message"
        @confirm="confirmation.method" @close="closeConfirmation" />
    <Modal :show="modal.open" @close="closeModal">
        <NewBusiness v-if="modal.component == 'EditBusiness'" @close="closeModal" :edit="true"
            :editData="currentBusiness" @notificationUpdate="openNotification" />
        <PaymentInformationForm v-if="modal.component == 'Add Payments'" @close="closeModal"
            @updateOrCreate="createOrUpdate" :paymentMethods="paymentMethodsStore.methods" />
    </Modal>
    <AuthenticatedLayout>
        <div class="text-2xl font-extrabold text-slate-800 dark:text-slate-50 mb-6">
            Business Information
        </div>
        <div class="container mx-auto p-5">
            <div class="flex flex-col gap-4">
                <!-- Business Details Section -->
                <div class="bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-50 p-6 rounded-lg shadow-md">
                        ...
                        <div class="flex flex-col lg:flex-row">
                            <div class="lg:w-5/12 w-full">
                                <img :src="
                                    ...
                                    " class="h-60 w-60 rounded-sm" alt="Business Logo" />
                                <div class="flex flex-col mt-4">
                                    <h2 class="text-2xl font-semibold text-slate-500 dark:text-slate-400">
                                        {{
                                        myBusiness?.business?.business
                                        ?.business_name
                                        }}
                            ...
                            <div class="flex flex-col space-y-6 text-lg mt-4 lg:mt-0 lg:ml-8">
                ...
                <div class="bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-50 p-6 rounded-lg shadow-md">
                    <h2 class="text-xl font-semibold text-rose-500 mb-4">
                        Other Details
                    </h2>
                    <div class="flex items-center space-x-2">
                    ...
                    <div class="flex items-center space-x-2 mt-2">
                        ...
                        <p class="font-semibold text-slate-500 dark:text-slate-400">Industry:</p>
                        <p>
                    ...
                    <div class="flex items-center space-x-2 mt-2">
                        <i class="pi pi-dollar" />
                        <p class="font-semibold text-slate-500 dark:text-slate-400">Currency:</p>
                        <p>
                    ...
                    <div class="flex items-center space-x-2 mt-2">
                        <i class="pi pi-check" />
                        <p class="font-semibold text-slate-500 dark:text-slate-400">Status:</p>
                        <Badge :severity="
                                getStatusSeverity(
                                    myBusiness?.business?.business?.status
                                )
                            " class="px-4 py-2 text-lg" style="text-transform: capitalize">
                            {{ myBusiness?.business?.business?.status }}
                        </Badge>
                    </div>
                </div>
                ...
                <div class="bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-50 p-6 rounded-lg shadow-md">
                    ...
                    <div v-if="
                        ...
                        <div v-for="(
                                item, index
                            ) in paymentMethodsStore?.paymentInformations" :key="index"
                            class="border flex-1 md:flex-none p-4 rounded-md shadow-md bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100">
                            ...
                            <ul class="mt-2">
                                <li v-for="(
                                        detail, detailIndex
                                    ) in item.payment_details" :key="detailIndex">
                    ...
                    <div v-else>
                        ...
                        <p class="text-rose-600 dark:text-rose-400 font-bold text-md mt-2">
