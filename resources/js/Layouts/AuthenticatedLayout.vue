...
<template>
    <Modal :show="modal.open" :closeable="modal.closeable" @close="closeModal">
        <NewBusiness v-if="modal.components == 'NewBusiness'" @close="closeModal" />
        <NoActiveSubscription v-if="modal.components == 'NoActiveSubscription'"
            :business="$page.props.user_businesses.default_business" />
    </Modal>
    <div>
        <div
            class="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-950 dark:text-white relative">
            ...
            <div class="fixed inset-0 pointer-events-none -z-10">
                ...
                <div
                    class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/2 h-1/2 bg-gradient-to-t from-blue-500/5 to-emerald-500/5 rounded-full blur-3xl animate-spin-slowest">
                </div>
                <!-- Animated dots/circles -->
                <div class="absolute top-20 right-40 w-2 h-2 bg-rose-500/30 rounded-full animate-ping"></div>
                <div class="absolute top-40 left-1/4 w-3 h-3 bg-amber-500/20 rounded-full animate-ping"
                    style="animation-delay: 1s"></div>
                <div class="absolute bottom-32 right-1/4 w-2.5 h-2.5 bg-blue-500/20 rounded-full animate-ping"
            ...
            <nav :class=" [
                ...
                <div
                    class="flex flex-row items-center h-fit border-b border-slate-200/70 dark:border-slate-700/70 px-2">
                    <div class="relative w-full bg-slate-50 dark:bg-slate-800 text-slate-950 dark:text-white">
                        <div class="card flex justify-center w-full">
                            <Select v-model="default_business" :options="businesses" class="w-full"
                                ...
                                <template #value="slotProps">
                                    <div v-if="slotProps.value" class="flex items-center relative">
                                        <div class="relative w-10 h-10 mr-3 flex-shrink-0">
                                            <div
                                                class="absolute inset-0 bg-gradient-to-br from-rose-400 to-purple-500 rounded-lg animate-pulse-slow">
                                            </div>
                                            <img :src="getImageUrl(slotProps.value.logo, '/images/default-business-logo.png')"
                                                alt="Business Logo"
                                                class="w-full h-full object-cover rounded-lg shadow-sm relative z-10" />
                                            <div
                                                class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-slate-50 dark:border-slate-800 z-20">
                                            </div>
                                        </div>
                                        <div class="flex flex-col">
                                            <span
                                                class="font-medium text-ellipsis whitespace-nowrap w-[180px] overflow-hidden">
                                                {{ slotProps.value.business_name }}
                                            </span>
                                            <span
                                                class="text-xs text-ellipsis whitespace-nowrap w-[180px] overflow-hidden text-slate-500 dark:text-slate-400">
                                ...
                                <template #option="slotProps">
                                    <div class="flex items-center w-full h-full gap-1 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer relative rounded-lg transition-all duration-150 my-1"
                                        @click="changeBusiness(slotProps.option)">
                                        <div class="relative">
                                            <div
                                                class="absolute inset-0 bg-gradient-to-br from-rose-400/20 to-purple-500/20 rounded-lg animate-pulse-slow">
                                            </div>
                                            <img :src="getImageUrl(slotProps.option.logo, '/images/default-business-logo.png')"
                                                alt="Business Logo"
                                        ...
                                        <div class="flex flex-col">
                                            ...
                                            <span
                                                class="text-xs text-slate-500 dark:text-slate-400 text-ellipsis whitespace-nowrap w-[200px] overflow-hidden">
                                                {{ slotProps.option.email }}
                                            </span>
                                        </div>
                                        <div v-if="slotProps.option.business_id == default_business.business_id"
                                ...
                                <template #footer>
                                    <div class="p-4 border-t border-slate-200 dark:border-slate-700"
                                        ...
                                        <Button label="Add New Business"
                                            ...
                                            size="small" icon="pi pi-plus" @click="createNewBusiness" />
                                    </div>
                                </template>
                            </Select>
                        </div>
                ...
                <div
                    class="nav-bar-holder flex flex-col h-[70vh] no-scrollbar overflow-y-scroll hide-overflow px-1 py-2">
                    <SideNavigations />
                </div>

                <!-- User Profile Section -->
                <div
                    class="profile h-fit flex items-center gap-3 relative border-t border-slate-200 dark:border-slate-700 bg-slate-50/80 px-2 dark:bg-slate-800/80 backdrop-blur-sm">
                    <div class="flex items-center bg-gradient-to-r from-slate-100 to-slate-50 dark:from-slate-700 dark:to-slate-800 rounded-lg h-fit w-full cursor-pointer gap-3 p-2 shadow-lg hover:shadow-xl transition-all duration-200 group"
                        @click="toggle">
                        <div class="relative">
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-blue-400/30 to-purple-500/30 rounded-full animate-pulse-slow">
                            </div>
                            <div
                                class="w-14 h-14 rounded-full overflow-hidden border-2 border-slate-200 dark:border-slate-600 shadow-inner relative z-10 group-hover:scale-105 transition-transform duration-200">
                                <img :src="getImageUrl($page.props.auth.user.profile_image, '/images/default-profile.png')"
                                    class="w-full h-full object-cover" />
                            </div>
                            <div
                                class="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-slate-100 dark:border-slate-700 z-20">
                            </div>
                        </div>
                        <div class="flex flex-col text-ellipsis h-auto w-full">
                            <div
                                class="text-base font-semibold h-6 w-36 truncate text-ellipsis whitespace-nowrap group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors duration-200">
                                {{ $page.props.auth.user.name }}
                            </div>
                            <div
                                class="text-xs text-slate-500 dark:text-slate-400 h-6 w-36 truncate text-ellipsis whitespace-nowrap">
                                {{ $page.props.auth.user.email }}
                            </div>
                        </div>
                        <div
                            class="text-slate-400 group-hover:text-rose-500 dark:group-hover:text-rose-400 transition-colors duration-200">
                            <i
                                class="pi pi-chevron-up text-sm group-hover:rotate-180 transition-transform duration-300"></i>
                        </div>
                    </div>

                    <Popover ref="op"
                        class="user-popover w-64 ms-3 p-0 shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                        <div
                            class="p-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-800 rounded-lg">
                        ...
                        <div class="py-2 bg-slate-50 dark:bg-slate-800">
                            <div class="px-3 py-2 text-xs font-semibold uppercase text-slate-400 dark:text-slate-500">
                                Account</div>

                            <DropdownLink :href="route('profile.edit')"
                                class="flex items-center gap-3 px-4 py-3 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors duration-150">
                                <div
                            ...
                            <DropdownLink :href="route('logout')" method="post" as="button"
                                ...
                                <div
                                    class="w-8 h-8 rounded-full bg-rose-100 dark:bg-rose-900/30 flex items-center justify-center">
                ...
                <div class="flex justify-between items-center h-full px-5">
                    <div class="flex items-center gap-4">
                        <button
                            ...
                            <i v-if="menuOpen"
                                class="pi pi-angle-left text-xl text-slate-500 dark:text-slate-400 group-hover:text-rose-500 dark:group-hover:text-rose-400 transition-colors duration-200"></i>
                            <i v-if="!menuOpen"
                                class="pi pi-angle-right text-xl text-slate-500 dark:text-slate-400 group-hover:text-rose-500 dark:group-hover:text-rose-400 transition-colors duration-200"></i>
                        </button>
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        <button
                            class="flex items-center justify-center w-8 h-8 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors duration-200 relative">
                        ...
                        <button
                            class="flex items-center justify-center w-8 h-8 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors duration-200">
                        ...
                        <button @click="toggleDarkMode"
                            ...
                            <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-200" fill="none"
            ...
            <main :class=" [
                ...
                <div class="pt-6 pb-8 h-full">
                    <div class="w-full mx-auto sm:px-6 lg:px-8">
                        <div
                            class="bg-slate-50/80 dark:bg-slate-800/80 backdrop-blur-md min-h-[92vh] h-fit shadow-xl sm:rounded-2xl p-6 border border-slate-200/50 dark:border-slate-700/50 relative overflow-hidden">
                            <!-- Decorative elements for content area -->
                            <div
                                class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-rose-400/5 to-blue-500/5 rounded-full blur-3xl -z-10">
                            </div>
                            <div
                                class="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tr from-purple-400/5 to-amber-500/5 rounded-full blur-3xl -z-10">
                            </div>
